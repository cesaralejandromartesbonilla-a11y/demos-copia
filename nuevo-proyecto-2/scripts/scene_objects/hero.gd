...
func _unhandled_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("forward") || Input.is_action_just_pressed("backward") || Input.is_action_just_pressed("move_left") || Input.is_action_just_pressed("move_right"):
		is_walking = true
		$godotbot/AnimationPlayer.speed_scale = speed
		$godotbot/AnimationPlayer.play("godotbot_walk/Walk")
	if event.is_released():
		if not Input.is_action_pressed("forward") and not Input.is_action_pressed("backward") and not Input.is_action_pressed("move_left") and not Input.is_action_pressed("move_right"):
			is_walking = false
