...
func _move_3d(delta: float, input_dir: Vector2, speed: float, drag: float, auto_forward: bool):
	...
	if auto_forward:
		...
		direction += cam_basis.x * input_dir.x * 0.5
	else:
...
# Función para que el InteractionManager o la comida la llamen
func consume_food_water(food_amount: float, water_amount: float):
	if survival:
		survival.current_hunger = min(survival.current_hunger + food_amount, survival.max_hunger)
		survival.current_thirst = min(survival.current_thirst + water_amount, survival.max_thirst)
		print("Consumido. Hambre: ", survival.current_hunger, " Sed: ", survival.current_thirst)
